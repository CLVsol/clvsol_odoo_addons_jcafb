...
class LabTestResultEdit(models.TransientModel):
    ...
    def _default_EUR19_data_entrada_material(self):
        return self._get_default('EUR19', 'EUR19-01-01')
    EUR19_data_entrada_material = fields.Date(
        'Data de Entrada do Material', readonly=False, default=_default_EUR19_data_entrada_material
    )

    def _write_EUR19_data_entrada_material(self):
        self._set_result('EUR19', 'EUR19-01-01', self.EUR19_data_entrada_material)

    def _default_EUR19_liberacao_resultado(self):
        return self._get_default('EUR19', 'EUR19-01-02')
    EUR19_liberacao_resultado = fields.Date(
        'Liberação do Resultado', readonly=False, default=_default_EUR19_liberacao_resultado
    )

    def _write_EUR19_liberacao_resultado(self):
        self._set_result('EUR19', 'EUR19-01-02', self.EUR19_liberacao_resultado)

    def _default_EUR19_examinador(self):
        employee_model = self.env['hr.employee']
        code = self._get_default('EUR19', 'EUR19-01-03')
        if code is not False:
            ...
            return False
    EUR19_examinador = fields.Many2one(
        'hr.employee',
        string='Examinador',
        readonly=False,
        default=_default_EUR19_examinador
    )

    def _write_EUR19_examinador(self):
        if self.EUR19_examinador.name is not False:
            self._set_result(
                'EUR19', 'EUR19-01-03',
                self.EUR19_examinador.name + ' [' + self.EUR19_examinador.code + ']'
            )
        else:
            self._set_result('EUR19', 'EUR19-01-03', False)
    ...
    def _do_result_updt_EUR19(self):

        self._write_EUR19_data_entrada_material()
        self._write_EUR19_liberacao_resultado()
        self._write_EUR19_examinador()
