...
class LabTestReport(models.Model):
    ...
    def lab_test_report_export_xls(self, dir_path, file_name, use_template, template_dir_path):
        ...
        if lab_test_type == 'EAN19':
            self.lab_test_report_export_xls_EAN19(sheet, row_nr, logo_file_path, use_template)
            save_book = True

        if lab_test_type == 'ECP19':
            self.lab_test_report_export_xls_ECP19(sheet, row_nr, logo_file_path, use_template)
            save_book = True

        if lab_test_type == 'EDH19':
            self.lab_test_report_export_xls_EDH19(sheet, row_nr, logo_file_path, use_template)
            save_book = True

        if lab_test_type == 'EEV19':
            self.lab_test_report_export_xls_EEV19(sheet, row_nr, logo_file_path, use_template)
            save_book = True

        if lab_test_type == 'EUR19':
            self.lab_test_report_export_xls_EUR19(sheet, row_nr, logo_file_path, use_template)
            save_book = True
