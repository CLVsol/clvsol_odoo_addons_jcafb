...
class LabTestResult(models.Model):
    ...
    def lab_test_result_export_xls(self, dir_path, file_name, use_template, template_dir_path):
        ...
        if lab_test_type == 'ECP19':
            self.lab_test_result_export_xls_ECP19(sheet, row_nr, use_template)
            save_book = True
        ...
        if lab_test_type == 'EEV19':
            self.lab_test_result_export_xls_EEV19(sheet, row_nr, use_template)
            save_book = True

        if lab_test_type == 'EUR19':
            self.lab_test_result_export_xls_EUR19(sheet, row_nr, use_template)
            save_book = True
