...
class LabTestOffReport(models.Model):
    ...
    def lab_test_off_report_export_xls(self, dir_path, file_name, use_template, template_dir_path):
        ...
        if use_template:
            ...
            file_name = file_name.replace('<type>', lab_test_type).replace('<request_code>', lab_test_off_request_code)
        ...
        _logger.info(u'%s %s %s %s', '>>>>>>>>>>', lab_test_off_request_code, lab_test_type, use_template)
        ...
        if lab_test_type == 'EAN19':
            self.lab_test_off_report_export_xls_EAN19(sheet, row_nr, logo_file_path, use_template)
            save_book = True

        if lab_test_type == 'EDH19':
            self.lab_test_off_report_export_xls_EDH19(sheet, row_nr, logo_file_path, use_template)
            save_book = True
