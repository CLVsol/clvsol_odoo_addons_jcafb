...
class DocumentOff(models.Model):
    ...
    def change_state(self, draft_state):
        for document in self:
            if document.is_allowed_transition(document.state, draft_state):
                document.state = draft_state
            else:
                raise UserError('Status transition (' + document.state + ', ' + draft_state + ') is not allowed!')
    ...
    def action_cancelled(self):
        for document in self:
            document.change_state('cancelled')
